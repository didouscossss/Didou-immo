...
class MarcheScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        for (final id in state.visibleSections(AppTab.marche)) ..._buildSection(id, state),
      ],
    );
  }
  ...
  List<Widget> _buildSection(String id, RendementState state) {
    switch (id) {
      case 'reperes':
        return [_sectionReperes(state)];
      case 'score':
        return [_sectionScore(state)];
      default:
        return const [];
    }
  }

  Widget _sectionReperes(RendementState state) {
  ...
  Widget _sectionScore(RendementState state) {
